...
export async function POST(req) {
  ...
  try {
    ...
    const [user] = await connection.query(`SELECT * FROM account WHERE gmail = ?`, [email]);
    if (user.length === 0) {
    ...
    await connection.query(sqlSaveToken, [email, token, expiresAt, token, expiresAt]);
      ...
      `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; background-color: #f8f9fa; border-radius: 5px;">
          <div style="text-align: center; margin-bottom: 20px;">
            <img src="${process.env.DOMAIN}/imgs/logo.gif" alt="Logo" style="width: 100px; height: auto;">
          </div>
